...
- **Modulo 04 — Construindo Tecnologia** em [/jornada/modulos/construindo-tecnologia](/jornada/modulos/construindo-tecnologia) — aprender a criar aplicativos sem codigo (no-code) usando IA na Lovable. 4 abas:
  ...
  - **Conectando uma base** — como o app para de perder os dados quando alguem limpa o navegador. Tres caminhos sem programar, com prompt pronto pra cada um: **Google Sheets** (a planilha segue sendo a fonte de verdade, o app le e grava de volta), **Lovable Cloud** (backend embutido, caminho mais curto) e **Supabase** (PostgreSQL com login e permissoes, com Table Editor em formato de planilha). Traz tabela de decisao e a coluna "quanto custa sair". Regra de ouro da aba: no prototipo, dado ficticio ou publico — os tres guardam dados fora do orgao, entao dado pessoal ou sigiloso passa antes pelo Modulo 06. Indica o BBSIA (bancobrasileiro.ia.br) pra achar base de teste. **Use quando**: pessoa pergunta como salvar os dados do app, como conectar planilha, como fazer login de usuario, ou diz que perdeu o que tinha feito.
...
- **Modulo 07 — Criando um Assistente** em [/jornada/modulos/criando-assistente](/jornada/modulos/criando-assistente) — do prompt ao assistente que fica. 5 abas:
  ...
  - **Passo a passo pra agentes** — onde criar um agente em cada uma das quatro plataformas mais usadas: no **Gemini** chama-se Gem (gratuito), no **ChatGPT** chama-se GPT, no **Claude** chama-se Project (gratuito, ate 5) e no **Copilot** chama-se agente. A tese da aba: o prompt e a parte transferivel, a interface e a parte descartavel — os tres templates funcionam igual nos quatro lugares, muda so onde colar. O Copilot tem o passo a passo completo de 9 passos na mesma aba. **Use quando**: pessoa pergunta como criar um Gem, um GPT, um Project ou um agente do Copilot, ou onde colar o template.
